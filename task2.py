...
# The vertex which has incoming edge, it will be incremented by 1
    for i in dict1:
        for j in dict1[i]:
            indegree[j]+=1
# The vertex which has no in-degree will be appended to the heapqueue first
    for j in range(1,len(indegree)):
        if indegree[j]==0:
            heapq.heappush(queue, j)    # This function is used to insert the element mentioned in its arguments into a queue.
    while queue:
        current_vertex=heapq.heappop(queue)     # This function is used to remove and return the smallest element from the queue.
        sorted_list.append(current_vertex)
        if current_vertex in dict1:
            for adjacency_vertex in dict1[current_vertex]:
                indegree[adjacency_vertex] -= 1
                if indegree[adjacency_vertex] == 0:
                    heapq.heappush(queue, adjacency_vertex)
